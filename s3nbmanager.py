...
def move_key(bucket, new_name, old_name):
	...
	bucket.copy_key(new_name, bucket.name, old_name)
	bucket.delete_key(old_key)

def new_key_from_string(bucket, name, contents):
	bucket.new_key(name).set_contents_from_string(contents)
...
class S3NotebookManager(NotebookManager):
	...
	def __init__(self, *args, **kwargs):
		...
		self.log.info("bucket_name = %s , bucket_folder = %s" %
					(self.bucket.name, self.notebook_dir))
	...
	def save_notebook(self, model, name='', path=''):
		...
		self.log.debug('File manager: saving notebook %s, %s' % (name, path))
		...
		if path != new_path or name != new_name:
			self.log.debug('renaming notebook %s %s->%s %s' (path, name, new_path, new_name))
