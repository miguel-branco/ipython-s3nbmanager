...
class S3NotebookManager(NotebookManager):
	...
	#CTM: we will need to define how we get the user id
	userid = ''

	#CTM: have to check why this is not working
#	aws_access_key =  Unicode('',config=True, help='AWS access key id.')
#	aws_secret_key =  Unicode('',config=True, help='AWS secret access key.')


	_bucket = get_notebooks_bucket(userid)
	# CTM we have to decide what do we want the default path to be
	...
	def _checkpoint_dir_default(self):
		#check if the self.notebook_dir has a '/' at the end	
	...
	def _checkpoint_dir_changed(self, name, old, new):
		...
		if (new.endswith('/') == False):
			new += '/'		
		if not key_exists(self._bucket, new):
			...
			try:
				...
				new_key_from_string(self._bucket, new, '')
			except:
	...
	def _copy(self, src, dest):
		...
		try:
			self._bucket.copy_key(dest, src_bucket_name, src)
	...
	def get_notebook_names(self, path=''):
		...
		self.log.info("getting nb names %s" %path)
		os_path = self._get_os_path(path = path)
					
		return list_keys(self._bucket, os_path, self.filename_ext)
	...
	def path_exists(self, path):
		...
		return is_folder(self._bucket, os_path)
	...
	def is_hidden(self, path):
		...
		"""			
		os_path = self._get_os_path(path=path)
		return is_hidden(self._bucket, os_path)
	...
	def notebook_exists(self, name, path=''):
		...
		return key_exists(self._bucket, os_path)
	...
	def list_dirs(self, path):
		...
		if not os_path.endswith('/'): os_path += '/'
		if(not key_exists(self._bucket, os_path)):
			self.log.info("path does not exist " + os_path )
			raise web.HTTPError(404, u'directory does not exist: %r' % os_path)
		elif(is_hidden(self._bucket, os_path)):
			self.log.info("path is hidden " + os_path )
			self.log.info("Refusing to serve hidden directory %s, via 404 Error" % os_path )
			raise web.HTTPError(404, u'directory does not exist: %s' % path)

		# the '/' makes it list dirs only (kind of) remember s3 has keys				
		dir_names =  list_keys(self._bucket, os_path, "/")
		dirs=[]

		for name in dir_names:		
			dir_path = self._get_os_path(name, path)
			self.log.info('checking folder %s' % dir_path)
			if self.should_list(dir_path)  and not is_hidden(self._bucket, dir_path):		
				model = self.get_dir_model(name, os_path)
				dirs.append(model)
	...
	def get_dir_model(self, name, path=''):
		...
		self.log.info("getting dir model %s"% os_path)
		key = self._bucket.get_key(os_path)
		
		#CTM: here we have to check UTC and the struct_time 
		#the original code was using tz.utcfromtimestamp(info.st_ctime)
		model ={}
		if(key == None):
			self.log.info("dir model %s %s not found"% (path, name))
		model['name'] = name
		model['path'] = path
	...
	def save_notebook(self, model, name='', path=''):
		...
		self.log.info("new name '%s/%s' '%s/%s'"% (new_path, new_name, path, name));
		if path != new_path or name != new_name:
			self.log.info('renaming notebook')
			self.rename_notebook(name, path, new_name, new_path)

		
		# Save the notebook file
		self.log.info('getting json content')
		os_path = self._get_os_path(new_name, new_path)	
		nb = current.to_notebook_json(model['content'])
		
		self.check_and_sign(nb, new_name, new_path)
		self.log.info("checked and signed")

		if 'name' in nb['metadata']:
			nb['metadata']['name'] = u''
		try:
			self.log.info("Autosaving notebook %s", os_path)
			new_key_from_string(self._bucket, os_path, current.writes(nb,  u'json'))
		...
		if self.save_script:
			...
			self.log.info("Writing script %s", py_path)
			try:
				
				new_key_from_string(self._bucket, py_path,current.writes(nb, u'py')) 
			except Exception as e:
				self.log.error(e)
	...
	def delete_notebook(self, name, path=''):
		...
		for checkpoint in self.list_checkpoints(name, path):
			...
			if key_exists(self._bucket,cp_path):
				self.log.debug("Unlinking checkpoint %s", cp_path)			
				self._bucket.delete_key(cp_path)
		
		self.log.debug("Unlinking notebook %s", os_path)
		self._bucket.delete_key(os_path)
	...
	def rename_notebook(self, old_name, old_path, new_name, new_path):
		...
		if key_exists(self._bucket, new_os_path):
		...
		if self.save_script:
			...
			if key_exists(self._bucket, new_py_path):
				raise web.HTTPError(409, u'Python script with name already exists: %s' % new_py_path)

		self.log.info('moving %s to %s',new_os_path,  old_os_path)

		try:
			move_key(self._bucket, new_os_path, old_os_path)
		except Exception as e:
			self.log.error(e)
			raise web.HTTPError(500, u'Unknown error renaming notebook: %s %s' % (old_os_path, e))

		self.log.info('moving check points')
		...
		for cp in old_checkpoints:
			...
			self.log.info("Renaming checkpoint %s -> %s", old_cp_path, new_cp_path)
			move_key(self._bucket, new_cp_path, old_cp_path)

		# Move the .py script
		if self.save_script:
			move_key(self._bucket, new_py_path, old_py_path)
	...
	def get_checkpoint_model(self, checkpoint_id, name, path=''):
		...
		path = path
		cp_path = self.get_checkpoint_path(checkpoint_id, name, path)
		key = self._bucket.get_key(cp_path)
	...
	def create_checkpoint(self, name, path=''):
		...
		if(not key_exists(self._bucket, self.checkpoint_dir)):
			new_key_from_string(self._bucket, self.checkpoint_dir, '')
	...
	def list_checkpoints(self, name, path=''):
		...
		if not  key_exists(self._bucket, path):
		    return []
		else:
		    return [self.get_checkpoint_model(checkpoint_id, name, path)]
	...
	def restore_checkpoint(self, checkpoint_id, name, path=''):
		...
		if not key_exists(self._bucket, cp_path):
